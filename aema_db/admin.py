...
class EtymologyInline(StackedInline):
    ...
    fieldsets = (
        ...
        ('Mandatory',{
            'fields':('type','description','comment','is_ogam','reference','language'),
            'classes':('collapse-open',)        
        }),        
        ('Core etymologies',{
            'fields':('ie_etymology','proto_celtic_etymology','celtic_etymology','celtic_cognates'),
            'classes':('collapse-open',)                     
        }),
        ('Etymologies A',{
            'fields':('cont_celtic','leptonic','other_hispano_celtic','gaulish'),
            'classes':('collapse-open',)                     
        }),        
        ('Etymologies B',{
            'fields':('insular_celtic','brittonic','welsh','cornish','cumbric',\
            'pictish_celtic','breton','goidelic','irish','scottish_gaelic','manx'),
            'classes':('collapse-open',)                     
        }),                
    )    
...
class OgamInscriptionAdmin(admin.OSMGeoAdmin):
    ...
    fieldsets = (
        ('General',{
            'fields':('ogamsite_fk','name','name_clean','toponym','script_fk'\
                ,'language_fk','language_of_inscription','reference','comment','chronology'\
                ,'materials'),
            'classes':('collapse-open',)        
        }),
        ('Relocated fields',{
            'fields':('leptonic','pie_etymology','ie_etymology','gaulish',\
                'proto_celtic_etymology','celtic_etymology','celtic_cognates','irish'\
                ,'galatian','morphology','syntax','semantic','other_hispano_celtic'\
                ,'phonology','script','welsh','cornish','breton','cont_celtic'\
                ,'cumbric','brittonic','insular_celtic','pictish_celtic'\
...
class BurialIndividualMain(admin.OSMGeoAdmin):
    ...
    raw_id_fields = ('organic_material_fk','position_fk','grave_good_m2m','pot_m2m','burial')
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        if db_field.name == "grave_good_m2m":
            try:
                ...
                kwargs["queryset"] = GraveGood.objects.filter(burial_id=parent_id)
            except ValueError:
                #  By default return an empty queryset
                kwargs["queryset"] = GraveGood.objects.none()
            return super(BurialIndividualAdmin, self).formfield_for_manytomany(db_field, request, **kwargs)               
...
class BurialAdmin(admin.OSMGeoAdmin):
    ...
    search_fields = ('site_fk__site_name','date_ref','site_type_specific__description')
    filter_horizontal = ('site_type_m2m','site_type_specific','gender_m2m','grave_good_class_m2m')
    ...
    fieldsets = (
        ...
        ('Site details',{
            'fields':('site_type_m2m','site_type_specific','cemetery','usage','funerary_context',\
                         'minimum_no_individuals','biblio_for_site'),
            'classes':('collapse-closed',)                     
        }),
        ('Burial details',{
            'fields':('burial_type','burial_summary','skeleton_summary','gender_m2m','age',),
            'classes':('collapse-closed',)                     
        }),
        ('Dating',{
            'fields':('general_chronology','general_chronology_fk','organic_material_fk','date_ref','date_summary','calibrated_date_bc_2_sigma','uncalibrated_date_bp','uncalibrated_date_bc',\
            'calibrated_date_bc_2_sigma_repeated_field'),
            'classes':('collapse-closed',)                     
        }),                      
        ('Grave goods',{
            'fields':('grave_good_short_summary','grave_good_class_m2m','no_of_grave_goods','pot_type','pot_type_summary'),
            'classes':('collapse-closed',)                     
        }),
    )
    inlines = [BurialImageInline,PotInlineAdmin,GraveGoodInlineAdmin,BurialIndividualAdmin,DatingInLine,]
    class Media:
        geocodefield = 'point'
...
class OgamAdmin(admin.OSMGeoAdmin):
    list_display = ('id','site_fk','ancient_name','number','script_fk','language_fk')
    search_fields = ('site_fk__site_name','province','number','text')
    filter_horizontal = ('name_forms_present',)
    fieldsets = (
        ('Location',{
            'fields':('site','site_fk','ancient_name','point','lat','lon','coords'),
            'classes':('collapse-open',)        
        }),
        ('Details',{
            'fields':('context','script_fk','language_fk','language','text','expansion','contains_pn_bool',\
...
class OgamSiteAdmin(admin.OSMGeoAdmin):
    ...
    fieldsets = (
        ('Location',{
            'fields':('site','province','site_fk','ancient_name','point','lat','lon','coords'),
            'classes':('collapse-open',)        
        }),
        ('Details context',{
            'fields':('full_text','expanded_text',#'meaning','etymology',\
			    'context_description','context_macro','context_macro_free_text',\
                'context_micro','context_micro_free_text','context_secondary','context_secondary_text'),
            'classes':('collapse-closed',)                     
        }),
    )
...
class StelaeAdmin(admin.OSMGeoAdmin):
    ...
    filter_horizontal = ('stone_shape_m2m','technique_m2m','location_summary',\
                         'type_of_land','stelae_type','human_figure_m2m','motif')
    exclude = ('human_figure','human_figure_bool','lance_spear','lance_spear_bool','sword','sword_bool',\
               'shield','shield_bool','bow_and_arrow_bool','bow_and_arrow','wheeled_vehicle',\
               'wheeled_vehicle_bool','quadropeds_animals_bool','quadroped_bool','mirror',\
               'mirror_bool','fibula','fibula_bool','comb','comb_bool','ornaments','ornaments_bool','headdress',\
               'headdress_bool','helmet','helmet_bool','musical_instrument','musical_instrument_bool','circles_cup_and_rings','circles_cup_and_rings_bool')
    fieldsets = (
        ('Location',{
            'fields':('site_name','location_description','site_fk','area','region','lon','lat','point'),
            'classes':('collapse-open',)        
        }),
        ('Stelae details',{
            'fields':('general_description','stone_shape','stone_shape_m2m','condition','technique_of_creation','technique_m2m','stone_type_and_description',\
                'specific_stone_type','motif_overview',\
            	'no_of_motifs','summary_of_human_figure','musical_instrument_description',\
                'spearhead_location',\
                'helmet_type','sword_summary','shield_summary','shield_location',\
                'bow_and_arrow_description',\
                'quadroped_summary','vehicle_composition',\
                'circle_etc_description','other_motifs',\
                'unusual_motifs','re_use','re_use_bool','additional_features','additional_features_bool',\
                'disposition_of_human_in_relation_to_other_motifs'),
            'classes':('collapse-closed',)                     
        }),
        ('New fields',{
            'fields':('discovery','location_summary','summary_location',\
                      'stelae_type','stelae_type_raw','type_of_land','type_of_land_raw',\
                      'human_figure_m2m','human_figure_m2m_raw','motif'),
            'classes':('collapse-closed',)
        }),
        ('Chronology and Bibliography',{
            'fields':('bibliography','chronology','chronology_fk','chronology_start_bc','chronology_end_bc'),
            'classes':('collapse-closed',)                     
        }),
    )
    class Media:
...
class AbrazoFernandoRawAdmin(admin.OSMGeoAdmin):
    ...
    exclude = ('cont_celtic_2','pictish_celtic_2','scottish_gaelic_2','other_hispano_celtic_2')
    fieldsets = [
        ('General', 
        ...
        ('Linguistic', 
        	{'fields': ['phonology','morphology','syntax','semantic']}),
        ('Etymology',
        	{'fields':['pie_etymology','ie_etymology','proto_celtic_etymology','celtic_etymology','celtic_cognates']}),
        ('Language group 1',
        	{'fields':['cont_celtic','leptonic','other_hispano_celtic','gaulish','galatian']}),
        ('Language group 2',
        	{'fields':['insular_celtic','brittonic','welsh','cumbric','pictish_celtic',\
        	'breton','goidelic','irish','scottish_gaelic','manx','cornish']}),
...
class HoardsAdmin(admin.OSMGeoAdmin):
    filter_horizontal = ('material_type_fk','general_type_m2m','specific_object_type_m2m','context_summary_m2m')
    list_display = ('id','site_fk','area_related','object_condition_summary_choice')
    def area_related(self,obj):
        try:
            return obj.site_fk.area_fk
        except Exception:
            return ''
    area_related.admin_order_field  = 'site_fk__area_fk'
    search_fields = ('site_fk__site_name','area','region','object_condition_summary_choice')
    raw_id_fields = ('site_fk',)
    fieldsets = (
        ('Location',{
            'fields':('site_fk','lat','lon','point',),
        ...
        ('Details',{
            'fields':('deposit_overview','deposit_context','no_of_objects_cleaned','no_of_objects_clustered_choice'\
                      ,'find_type_cleaned','material_type_fk','general_type_m2m','specific_object_description'\
                      ,'specific_object_type_m2m','arrangement_of_objects','object_condition_summary_choice'\
                      ,'dry_wet_context_choice','metal_analysis_summary_1','context_summary','context_summary_m2m','notes'),
            'classes':('collapse-closed',)                     
        }),
        ('Chronology and Bibliography',{
            'fields':('chronology_date_bracket_BC','chronology_start_date_BC','chronoloy_end_date_BC'\
                     ,'regional_period_phase','period_summary_fk','c14_dates','refs'),
            'classes':('collapse-closed',)                     
        }),
    )
    ...
    fieldsets = (
        ...
        ('Details',{
            'fields':('deposit_overview','deposit_context','no_of_objects_cleaned','no_of_objects_clustered_choice'\
                      ,'find_type_cleaned','material_type_fk','general_type_m2m','specific_object_description'\
                      ,'specific_object_type_m2m','arrangement_of_objects','object_condition_summary_choice'\
                      ,'dry_wet_context_choice','material_analysis_summary','context_summary','context_summary_m2m','notes'),
            'classes':('collapse-closed',)
        }),
        ('Chronology and Bibliography',{
            'fields':('chronology_date_bracket_BC','chronology_start_date_BC','chronoloy_end_date_BC'\
...
admin.site.register(GraveGoodPlacement, admin.GeoModelAdmin,exclude=('short_desc',))
