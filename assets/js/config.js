...
var chrono = [
  ...
  "Beaker-Middle Bronze Age ",
  "Beaker-Late Bronze Age ",
  "Early Bronze Age ",
...
function sortByChronology() {
  var myList = $('h4:contains("general chronology")')
    .next("div")
    .children("ul");
  ...
  var myList = $("#facet-individuals")
    .find('h4:contains("chronology")')
    .next("div")
    .children("ul");
  var listItems = myList.children("li").get();
  listItems.sort(function (a, b) {
  ...
  var myList = $("#facet-settlements")
    .find('h4:contains("period]")')
    .next("div")
    .children("ul");
  var listItems = myList.children("li").get();
  listItems.sort(function (a, b) {
...
function updateFacetedResults() {
  ...
  for (sf in selectedFacets) {
    ...
    if (selectedFacets[sf].type.indexOf(lastFacetedObjectType) != -1) {
      ...
      selectedURL +=
        "&selected_filters=" +
        selectedFacets[sf].type +
        ":" +
        selectedFacets[sf].value.replace(/[\/()><]/g, " ");
    }
  }
  ...
  for (sof in selectedOrFacets) {
    ...
    if (selectedOrFacets[sof].type.indexOf(lastFacetedObjectType) != -1) {
      ...
      selectedOrURL +=
        "&selected_facets=" +
        selectedOrFacets[sof].type +
        ":" +
        selectedOrFacets[sof].value.replace(/[\/()><]/g, " ");
    }
  }
  ...
  for (ef in excludedFacets) {
    ...
    if (excludedFacets[ef].type.indexOf(lastFacetedObjectType) != -1) {
      ...
      excludedURL +=
        "&deselected_filters=" +
        excludedFacets[ef].type +
        ":" +
        excludedFacets[ef].value.replace(/[\/(),><]/g, " ");
    }
  }
  ...
  window[lastFacetedObjectType + "Query"] =
    selectedURL + selectedOrURL + excludedURL;
      ...
      success: function (data) {
        ...
        for (f in selectedURL.split("&selected_filters=")) {
          ...
          if (currFacet.indexOf(":") != -1) {
            ...
            andStr +=
              "<li>" +
              toTitleCase(subType.replace(/_/g, " ")) +
              ': <a href="#" class="green button radius less-padding"><i class="fa fa-check-circle"></i> ' +
              facet_value +
              "</a>";
        ...
        for (f in selectedOrURL.split("&selected_facets=")) {
          ...
          if (currFacet.indexOf(":") != -1) {
            ...
            orStr +=
              "<li>" +
              toTitleCase(subType.replace(/_/g, " ")) +
              ': <a href="#" class="yellow button radius less-padding"><i class="fa fa-plus-circle"></i> ' +
              facet_value +
              "</a>";
          ...
          if (currFacet.indexOf(":") != -1) {
            ...
            if (language == "es") {
              notStr +=
                "<li>" +
                toTitleCase(subType.replace(/_/g, " ")) +
                ' no es: <a href="#" class="red button radius less-padding"><i class="fa fa-times-circle"></i> ' +
                facet_value +
                "</a>";
            } else {
              notStr +=
                "<li>" +
                toTitleCase(subType.replace(/_/g, " ")) +
                ' not: <a href="#" class="red button radius less-padding"><i class="fa fa-times-circle"></i> ' +
                facet_value +
                "</a>";
            }
  ...
  // Map
  $.ajax(
    "/search/map/?&mime_type=application/json" +
      selectedURL +
      selectedOrURL +
      excludedURL +
      /* datequeryURL + */ querytypeURL,
...
function parseMapResults(data) {
  ...
  for (r in geoJsonResults.features) {
    ...
    marker_layer =
      window[geoJsonResults.features[r].properties.layer + "_markers"];
  ...
  $("#" + lastFacetedObjectType + "-json").val(
    JSON.stringify(window[lastFacetedObjectType].toGeoJSON())
  );
...
function clearFacets(layertoclear) {
  ...
  $('.facet-select[object-type="' + lastFacetedObjectType + '"]').each(
    function () {
      $(this).attr("checked", false);
    }
  );
  $('.facet-or-select[object-type="' + lastFacetedObjectType + '"]').each(
    function () {
      $(this).attr("checked", false);
    }
  );
  $('.facet-exclude[object-type="' + lastFacetedObjectType + '"]').each(
    function () {
      $(this).attr("checked", false);
    }
  );
...
$(document).ready(function () {
  ...
  graticules = L.graticule({
    style: { color: "rgb(0,0,255)", weight: 0.5, opacity: 0.45 },
    interval: 5,
  }).addTo(resultMap);

  labelGraticules();

  ancient_world = new L.TileLayer(
    "http://{s}.tiles.mapbox.com/v3/isawnyu.map-knmctlkh/{z}/{x}/{y}.png",
    { attribution: 'Map base by <a href="http://awmc.unc.edu/">AWMC</a>' }
  ).addTo(resultMap);

  legacy_terrain = new L.TileLayer(
    "/geoserver/gwc/service/tms/1.0.0/Project_data%3Adcow_backdrop_aema@EPSG%3A900913@jpeg/{z}/{x}/{y}.jpeg",
    { tms: true }
  ); //.addTo(resultMap);

  plain_map = new L.TileLayer(
    "/geoserver/gwc/service/tms/1.0.0/world_simple@EPSG%3A900913@png/{z}/{x}/{y}.png",
    { tms: true }
  ); //.addTo(resultMap);
  ...
  landscape = new L.TileLayer(
    "http://{s}.tile.thunderforest.com/landscape/{z}/{x}/{y}.png",
    {
      subdomains: ["a", "b", "c"],
      attribution:
        'Map base by <a href="http://www.thunderforest.com/">Thunderforest</a>',
    }
  );

  ba_roads = new L.TileLayer(
    "/geoserver/gwc/service/tms/1.0.0/aema_wms:ba_roads@EPSG%3A900913@png/{z}/{x}/{y}.png",
    { tms: true, transparent: true }
  ); //.addTo(resultMap);

  miniBase = new L.TileLayer(
    "http://{s}.acetate.geoiq.com/tiles/acetate-base/{z}/{x}/{y}.png",
    { subdomains: ["a1", "a2", "a3", "a4"], maxzoom: 5 }
  );

  var esri_streets = new L.TileLayer(
  ...
  burials = new L.geoJson(fakeObject, {
  ...
  miscellaneous = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      /*	
			var marker = L.marker(latlng,{icon:burialIcon});
			*/
  ...
  gravegoods = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      /*  
                        var marker = L.marker(latlng,{icon:burialIcon});
                        */
      //var marker = L.circleMarker(latlng,{opacity:0.9,fillOpacity:0.9}).setRadius(5) // Basic blue marker
      var marker = L.circleMarker(latlng, markerStyles["gravegoods"]).setRadius(
        5
      );
      //marker.bindPopup(feature.properties.popupContent);
      //marker.on('click',function(){
  ...
  hoards = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      /*
  ...
  metalwork = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      /*
			var marker = L.marker(latlng,{icon:hoardIcon});
			*/
  ...
  toponyms = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      var marker = L.circleMarker(latlng, {
        color: "#cccc33",
        opacity: 0.9,
  ...
  settlements = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      /*
                        var marker = L.marker(latlng,{icon:hoardIcon});
                        */
  ...
  stelae = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      /*
  ...
  inscriptions = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      var marker = L.circleMarker(latlng, {
        color: "#b16211",
        opacity: 0.9,
  ...
  individuals = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      var marker = L.circleMarker(latlng, {
        color: "purple",
        opacity: 0.9,
  ...
  text_search_layer = new L.geoJson(fakeObject, {
    pointToLayer: function (feature, latlng) {
      var marker = L.circleMarker(
        latlng,
        markerStyles[feature.properties.layer]
      ).setRadius(5);
      marker.bindPopup(feature.properties.popupContent);
      return marker;
    },
  ...
  layerSwitcher = new L.Control.Layers(base_layers, {
    Burials: burials_markers,
    ...
    Metalwork: metalwork_markers,
    Stelae: stelae_markers,
    Inscriptions: inscriptions_markers,
    "Grave goods": gravegoods_markers,
    "Buried individuals": individuals_markers,
    Pots: pots_markers,
    Toponyms: toponyms_markers,
    Settlements: settlements_markers,
    "Roman roads": ba_roads,
  }).addTo(resultMap);
...
function textSearchInitialise() {
  ...
  $("#text-search-submit").on("click", function () {
    var modelQ;
  ...
  $("#text-clear-submit").on("click", function () {
    updateTextResults("");
    $("#text-search").val("");
    textQuery = "";
  });
...
function intialiseFacetSearch() {
  ...
  $(".facet-select").on("click", function () {
    $(this)
      .parent()
      .parent()
      .parent()
  ...
  $(".facet-or-select").on("click", function () {
    $(this)
      .parent()
      .parent()
      .parent()
  ...
  // Event listeners for checkboxes
  $(".facet-exclude").on("click", function () {
    $(this)
      .parent()
      .parent()
      .parent()
...
var context_layers = [
  "regions_tribal_influence",
  "sites_ritual_iron_age_roman",
];

function prepareContextLayerList() {
  for (contextLayer in context_layers) {
    $("#context-layers").append(
      '<li><input class="layer-select" type="checkbox" layer-name="' +
...
function resultPaginate(pagenumber, query_type, query) {
  if (query_type == "text") {
    ...
    $.ajax(
      "/search/refine/?" +
        window[query_type + "Query"] +
        /* datequeryURL */ "&query_type=" +
        query_type +
        "&page=" +
        pagenumber,
      {
...
function reclusterLayer(layerToToggle) {
  ...
  layerSwitcher.addOverlay(
    window[layerToToggle + "_markers"],
    toTitleCase(layerToToggle)
  );
...
var icons = {
  swords: L.icon({
    iconUrl: "/static/map_icons/swords.svg",
    iconSize: [32, 32],
    iconAnchor: [16, 32],
  }),
  shields: L.icon({
    iconUrl: "/static/map_icons/shields.svg",
    iconSize: [24, 24],
    iconAnchor: [12, 24],
  }),
  coins: L.icon({
    iconUrl: "/static/map_icons/coins.svg",
    iconSize: [24, 24],
    iconAnchor: [12, 24],
  }),
  roman: L.icon({
    iconUrl: "/static/map_icons/roman.svg",
    iconSize: [24, 24],
    iconAnchor: [12, 24],
  }),
  "ritual sites": L.icon({
    iconUrl: "/static/map_icons/ritual_sites.svg",
    iconSize: [22, 22],
    iconAnchor: [11, 22],
  }),
  souterrain: L.icon({
    iconUrl: "/static/map_icons/souterrain.svg",
    iconSize: [20, 20],
    iconAnchor: [10, 20],
  }),
  "horse gear": L.icon({
    iconUrl: "/static/map_icons/horse.svg",
    iconSize: [32, 32],
    iconAnchor: [16, 32],
  }),
  axes: L.icon({
    iconUrl: "/static/map_icons/axes.svg",
    iconSize: [24, 24],
    iconAnchor: [1, 24],
  }),
  spears: L.icon({
    iconUrl: "/static/map_icons/spears.svg",
    iconSize: [24, 24],
    iconAnchor: [24, 24],
  }),
  sculptures: L.icon({
    iconUrl: "/static/map_icons/sculptures.svg",
    iconSize: [24, 24],
    iconAnchor: [1, 24],
  }),
  pictish: L.icon({
    iconUrl: "/static/map_icons/pictish.svg",
    iconSize: [32, 32],
    iconAnchor: [16, 24],
  }),
  "personal ornaments": L.icon({
    iconUrl: "/static/map_icons/armlet.svg",
    iconSize: [32, 32],
    iconAnchor: [16, 24],
  }),
};
...
function printMap() {
  ...
  $.each($(".result-container.content"), function () {
    if ($(this).find("ul.filters").length > 0) {
      $("#map-image-details").append(
        "<h3>" +
          toTitleCase($(this).attr("id").replace("-results", "")) +
          "</h3>"
      );
...
function extraCanvas() {
  ...
  kmScaleWidth = Math.ceil(
    $($(".leaflet-control-scale-line")[0]).css("width").replace("px", "")
  );
  kmValue =
    parseInt($($(".leaflet-control-scale-line")[0]).html().replace(" km", "")) *
    2;
  ...
  dtx.fillText(
    resultMap.getBounds()._northEast.lat.toFixed(3) +
      ", " +
      resultMap.getBounds()._southWest.lng.toFixed(3),
    10,
    15
  );
  ...
  dtx.fillRect(
    25 + Math.ceil(kmScaleWidth),
    resultMap.getSize().y - 25,
    Math.ceil(kmScaleWidth),
    1
  );
  dtx.fillRect(
    25 + Math.ceil(kmScaleWidth) + Math.ceil(kmScaleWidth),
    resultMap.getSize().y - 30,
    1,
    10
  );
}
...
function zoomToEngland() {
  resultMap.fitBounds(
    L.latLngBounds(
      [47.17477833929903, -8.349609375],
      [57.7862326105289, 4.7021484375]
    )
  );
}
...
function zoomToIberia() {
  resultMap.fitBounds(
    L.latLngBounds(
      [33.17434155100208, -10.8544921875],
      [46.55886030311719, 2.79052734375]
    )
  );
}
...
function zoomToFrance() {
  resultMap.fitBounds(
    L.latLngBounds(
      [40.38002840251183, -5.4931640625],
      [52.40241887397332, 7.55859375]
    )
  );
}
...
function zoomToIreland() {
  resultMap.fitBounds(
    L.latLngBounds(
      [50.91688748924508, -11.239013671875],
      [56.1210604250441, -4.713134765624999]
    )
  );
}
...
chronoClean = [];
chrono.forEach(function (id) {
  chronoClean.push(id.trim());
});
...
function pulseBurialsMarkers() {
  for (b in burials._layers) {
    if (
      chronoClean.indexOf(
        burials._layers[b].feature.properties.extra_properties[0][
          "_general_chronology"
        ]
      ) == timeSlice
    ) {
...
function pulseIndividualsMarkers() {
  for (b in individuals._layers) {
    if (
      chronoClean.indexOf(
        individuals._layers[b].feature.properties.extra_properties[0][
          "_chronology"
        ]
      ) == timeSlice
    ) {
...
function getLabel(m) {
  if (m.feature.properties["name"].indexOf(",") == -1) {
    ...
    return m.feature.properties["name"].substring(
      0,
      m.feature.properties["name"].indexOf(",")
    );
  }
}
